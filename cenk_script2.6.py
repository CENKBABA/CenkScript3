	...
	while satır<len(satırlar):
		...
		if "==" not in i and "!=" not in i and "=" in i:
			...
			if kntls==[]:
				...
				if satır in fnk_ifade:
					ifd=fnk_ifade[satır]
					#print("baban",ifd,fnk_çözülecek)
					ifd=ifd.replace(fnk_çözülecek,fnk_çözüm)
					#print("baban2",ifd)
					fnk_ifade[satır]=ifd
					knt2=fnk_ayıkla(ifd)
					if knt2==[]:
					...
					fnk_ifade.update({satır:değerle3(tt[1],değerler)})
					fnkad=kntls[0].split("(")[0]
					fnkpr=kntls[0][len(fnkad)+1:-1].split(",")
					#print(fnkad,fnkpr)
					fnk_çözülecek=kntls[0]
					sycc=0
					for i in fnkpr:
						ttt=fnk_parametreler[fonksiyonlar[fnkad]][sycc]
						değerler.update({ttt:_4işlem(parantez_çözücü(i,değerler))})
			...
			asdtt=i.split(" ")[1]
			satır=fnk_git[-1]-1
			fnk_çözüm=str(_4işlem(değerle3(parantez_çözücü(asdtt,değerler),değerler)))
...
#for i in fnk_ayıkla("100+üs((10-1)/(5-2),(10-8)*(5-3))+7+topla(10,3)"):
#	print(i)
...
sy1=faktöryel(topla(faktöryel(3),1))
